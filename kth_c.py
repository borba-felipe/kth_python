...
Original file is located at
    https://colab.research.google.com/drive/1_yMZMgbG17E8d8UCheYuMopzMfp8Vvjc
"""

# DataSet: KTH
# Treino da CNN: amostra c
# Validação do treino: amostras a, b e d
...
train_inputs=[]
train_labels=[]
data_dir_train='gdrive/My Drive/Amostras/amostra_c'
data_dir_val1='gdrive/My Drive/Amostras/amostra_a'
data_dir_val2='gdrive/My Drive/Amostras/amostra_b'
...
def checkFileExistance(filePath):
    try:
        ...
        return False
if checkFileExistance('/content/gdrive/My Drive/BancoDados/KTH_C_num.dat'):
    import sys
...
# KTH_C : lista que contará os dados para estatística dos classificadores
...
KTH_C_num = np.zeros(20, dtype=np.float64)
KTH_C_num[0] = best_acc
KTH_C_num[1] = xensemb
xvencedor=[]
tg = 2
...
for z in range(3):
  ...
  tg += 1
for z in range(3):
  KTH_C_num[tg] = xscor1[z]
  
  tg += 1
for z in range(3):
  KTH_C_num[tg] = et_scor[z]
  
  tg += 1
for z in range(3):
  KTH_C_num[tg] = es_scor[z]
  
  tg += 1
for z in range(3):
  KTH_C_num[tg] = mt_scor[z]
  
  tg += 1
for z in range(3):
  KTH_C_num[tg] = ms_scor[z]
  tg += 1
...
bs_total = as_total - 2
KTH_C_class = np.empty(bs_total, dtype=np.float64)
for i in range(bs_total):
    if xvencedor[i] == xnome_class[0]:
        KTH_C_class[i] = float(0.0)
    if xvencedor[i] == xnome_class[1]:
        KTH_C_class[i] = float(1.0)
    if xvencedor[i] == xnome_class[2]:
        KTH_C_class[i] = float(2.0)
    if xvencedor[i] == xnome_class[3]:
        KTH_C_class[i] = float(3.0)
    if xvencedor[i] == xnome_class[4]:
        KTH_C_class[i] = float(4.0)
    if xvencedor[i] == xnome_class[5]:
        KTH_C_class[i] = float(5.0)
    if xvencedor[i] == xnome_class[6]:
        KTH_C_class[i] = float(6.0)
    if xvencedor[i] == xnome_class[7]:
        KTH_C_class[i] = float(7.0)
    if xvencedor[i] == xnome_class[8]:
        KTH_C_class[i] = float(8.0)
    if xvencedor[i] == xnome_class[9]:
        KTH_C_class[i] = float(9.0)
cs_total = as_total - 2
KTH_C_qtd = np.zeros(18, dtype=np.float64)
KTH_C_qtd[0]=n_prim
KTH_C_qtd[1]=n_seg
KTH_C_qtd[2]=n_ter
KTH_C_qtd[3]=xn_prim
KTH_C_qtd[4]=xn_seg
KTH_C_qtd[5]=xn_ter
KTH_C_qtd[6]=et_n_prim
KTH_C_qtd[7]=et_n_seg
KTH_C_qtd[8]=et_n_ter
KTH_C_qtd[9]=es_n_prim
KTH_C_qtd[10]=es_n_seg
KTH_C_qtd[11]=es_n_ter
KTH_C_qtd[12]=mt_n_prim
KTH_C_qtd[13]=mt_n_seg
KTH_C_qtd[14]=mt_n_ter
KTH_C_qtd[15]=ms_n_prim
KTH_C_qtd[16]=ms_n_seg
KTH_C_qtd[17]=ms_n_ter
pprint(KTH_C_num)
pprint(KTH_C_class)
pprint(KTH_C_qtd)

arq_est_num = "/content/gdrive/My Drive/BancoDados/KTH_C_num.dat"
arq_est_class = "/content/gdrive/My Drive/BancoDados/KTH_C_class.dat"
arq_est_qtd = "/content/gdrive/My Drive/BancoDados/KTH_C_qtd.dat"
KTH_C_num.tofile(arq_est_num)
KTH_C_class.tofile(arq_est_class)
KTH_C_qtd.tofile(arq_est_qtd)

a_num = np.fromfile(arq_est_num)
b_class = np.fromfile(arq_est_class)
c_qtd = np.fromfile(arq_est_qtd)
pprint(a_num == KTH_C_num)
pprint(b_class == KTH_C_class)
pprint(c_qtd == KTH_C_qtd)
