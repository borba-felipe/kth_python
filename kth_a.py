...
Original file is located at
    https://colab.research.google.com/drive/1CeO-yXYRjDfzMpFm1Y0OaemQYn_fUeAy
"""

# DataSet: KTH
# Treino da CNN: amostra a
# Validação do treino: amostras b, c e d
...
def make_dataset(dir, class_to_idx, extensions=None, is_valid_file=None, dir2=None, dir3=None, dir4=None):
    images = []
    if (dir2 is None) and (dir3 is None) and (dir4 is None):
      ...
      for i in range(0,3):
        if i == 0:
          dir = os.path.expanduser(dir2)
        if i == 1:
          if dir3 is None:
            break
          dir = os.path.expanduser(dir3)
        if i == 2:
          if dir4 is None:
            break
          dir = os.path.expanduser(dir4)
        #print(dir)
        #xval = input('diretório atual: ')
        def is_valid_file(x):
          return has_file_allowed_extension(x, extensions)
        for target in sorted(class_to_idx.keys()):
          d = os.path.join(dir, target)
    ...
    #for i in images:
      #print(i)
    #pausa = input("digite algo")
    return images
 
class DatasetFolder(VisionDataset):
    def __init__(self, root, loader, extensions=None, transform=None,
                 target_transform=None, is_valid_file=None, dir2=None, dir3=None, dir4=None):
        ...
        self.dir3 = dir3
        self.dir4 = dir4
        samples = make_dataset(self.root, class_to_idx, extensions, is_valid_file, dir2, dir3, dir4)
        if len(samples) == 0:
...
class ImageFolder(DatasetFolder):
    ...
    def __init__(self, root, transform=None, target_transform=None,
                 loader=default_loader, is_valid_file=None, dir2=None, dir3=None, dir4=None):
        super(ImageFolder, self).__init__(root, loader, IMG_EXTENSIONS if is_valid_file is None else None,
                                          transform=transform,
                                          target_transform=target_transform,
                                          is_valid_file=is_valid_file, dir2=dir2, dir3=dir3, dir4=dir4)
...
train_inputs=[]
train_labels=[]
data_dir_train='gdrive/My Drive/Amostras/amostra_a'
data_dir_val1='gdrive/My Drive/Amostras/amostra_b'
data_dir_val2='gdrive/My Drive/Amostras/amostra_c'
data_dir_val3='gdrive/My Drive/Amostras/amostra_d'
image_datasets= {'train': ImageFolder(os.path.join(data_dir_train),data_transforms['train']),
                     'val': ImageFolder(os.path.join(data_dir_train),data_transforms['val'],
                                        dir2=os.path.join(data_dir_val1),dir3=os.path.join(data_dir_val2),dir4=os.path.join(data_dir_val3))}
...
def checkFileExistance(filePath):
    try:
        ...
        return False
if checkFileExistance('/content/gdrive/My Drive/BancoDados/KTH_A_num.dat'):
    import sys
...
# KTH_A : lista que contará os dados para estatística dos classificadores
...
KTH_A_num = np.zeros(20, dtype=np.float64)
KTH_A_num[0] = best_acc
KTH_A_num[1] = xensemb
xvencedor=[]
tg = 2
...
for z in range(3):
  ...
  tg += 1
for z in range(3):
  KTH_A_num[tg] = xscor1[z]
  
  tg += 1
for z in range(3):
  KTH_A_num[tg] = et_scor[z]
  
  tg += 1
for z in range(3):
  KTH_A_num[tg] = es_scor[z]
  
  tg += 1
for z in range(3):
  KTH_A_num[tg] = mt_scor[z]
  
  tg += 1
for z in range(3):
  KTH_A_num[tg] = ms_scor[z]
  tg += 1
...
bs_total = as_total - 2
KTH_A_class = np.empty(bs_total, dtype=np.float64)
for i in range(bs_total):
    if xvencedor[i] == xnome_class[0]:
        KTH_A_class[i] = float(0.0)
    if xvencedor[i] == xnome_class[1]:
        KTH_A_class[i] = float(1.0)
    if xvencedor[i] == xnome_class[2]:
        KTH_A_class[i] = float(2.0)
    if xvencedor[i] == xnome_class[3]:
        KTH_A_class[i] = float(3.0)
    if xvencedor[i] == xnome_class[4]:
        KTH_A_class[i] = float(4.0)
    if xvencedor[i] == xnome_class[5]:
        KTH_A_class[i] = float(5.0)
    if xvencedor[i] == xnome_class[6]:
        KTH_A_class[i] = float(6.0)
    if xvencedor[i] == xnome_class[7]:
        KTH_A_class[i] = float(7.0)
    if xvencedor[i] == xnome_class[8]:
        KTH_A_class[i] = float(8.0)
    if xvencedor[i] == xnome_class[9]:
        KTH_A_class[i] = float(9.0)
cs_total = as_total - 2
KTH_A_qtd = np.zeros(18, dtype=np.float64)
KTH_A_qtd[0]=n_prim
KTH_A_qtd[1]=n_seg
KTH_A_qtd[2]=n_ter
KTH_A_qtd[3]=xn_prim
KTH_A_qtd[4]=xn_seg
KTH_A_qtd[5]=xn_ter
KTH_A_qtd[6]=et_n_prim
KTH_A_qtd[7]=et_n_seg
KTH_A_qtd[8]=et_n_ter
KTH_A_qtd[9]=es_n_prim
KTH_A_qtd[10]=es_n_seg
KTH_A_qtd[11]=es_n_ter
KTH_A_qtd[12]=mt_n_prim
KTH_A_qtd[13]=mt_n_seg
KTH_A_qtd[14]=mt_n_ter
KTH_A_qtd[15]=ms_n_prim
KTH_A_qtd[16]=ms_n_seg
KTH_A_qtd[17]=ms_n_ter
pprint(KTH_A_num)
pprint(KTH_A_class)
pprint(KTH_A_qtd)

arq_est_num = "/content/gdrive/My Drive/BancoDados/KTH_A_num.dat"
arq_est_class = "/content/gdrive/My Drive/BancoDados/KTH_A_class.dat"
arq_est_qtd = "/content/gdrive/My Drive/BancoDados/KTH_A_qtd.dat"
KTH_A_num.tofile(arq_est_num)
KTH_A_class.tofile(arq_est_class)
KTH_A_qtd.tofile(arq_est_qtd)

a_num = np.fromfile(arq_est_num)
b_class = np.fromfile(arq_est_class)
c_qtd = np.fromfile(arq_est_qtd)
pprint(a_num == KTH_A_num)
pprint(b_class == KTH_A_class)
pprint(c_qtd == KTH_A_qtd)
