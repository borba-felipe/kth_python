...
# DataSet: KTH
# Treino da CNN: amostra b
# Validação do treino: amostras a, c e d
...
train_inputs=[]
train_labels=[]
data_dir_train='gdrive/My Drive/Amostras/amostra_b'
...
def checkFileExistance(filePath):
    try:
        ...
        return False
if checkFileExistance('/content/gdrive/My Drive/BancoDados/KTH_B_num.dat'):
    import sys
...
# KTH_B : lista que contará os dados para estatística dos classificadores
...
KTH_B_num = np.zeros(20, dtype=np.float64)
KTH_B_num[0] = best_acc
KTH_B_num[1] = xensemb
xvencedor=[]
tg = 2
...
for z in range(3):
  ...
  tg += 1
for z in range(3):
  KTH_B_num[tg] = xscor1[z]
  
  tg += 1
for z in range(3):
  KTH_B_num[tg] = et_scor[z]
  
  tg += 1
for z in range(3):
  KTH_B_num[tg] = es_scor[z]
  
  tg += 1
for z in range(3):
  KTH_B_num[tg] = mt_scor[z]
  
  tg += 1
for z in range(3):
  KTH_B_num[tg] = ms_scor[z]
  tg += 1
...
bs_total = as_total - 2
KTH_B_class = np.empty(bs_total, dtype=np.float64)
for i in range(bs_total):
    if xvencedor[i] == xnome_class[0]:
        KTH_B_class[i] = float(0.0)
    if xvencedor[i] == xnome_class[1]:
        KTH_B_class[i] = float(1.0)
    if xvencedor[i] == xnome_class[2]:
        KTH_B_class[i] = float(2.0)
    if xvencedor[i] == xnome_class[3]:
        KTH_B_class[i] = float(3.0)
    if xvencedor[i] == xnome_class[4]:
        KTH_B_class[i] = float(4.0)
    if xvencedor[i] == xnome_class[5]:
        KTH_B_class[i] = float(5.0)
    if xvencedor[i] == xnome_class[6]:
        KTH_B_class[i] = float(6.0)
    if xvencedor[i] == xnome_class[7]:
        KTH_B_class[i] = float(7.0)
    if xvencedor[i] == xnome_class[8]:
        KTH_B_class[i] = float(8.0)
    if xvencedor[i] == xnome_class[9]:
        KTH_B_class[i] = float(9.0)
cs_total = as_total - 2
KTH_B_qtd = np.zeros(18, dtype=np.float64)
KTH_B_qtd[0]=n_prim
KTH_B_qtd[1]=n_seg
KTH_B_qtd[2]=n_ter
KTH_B_qtd[3]=xn_prim
KTH_B_qtd[4]=xn_seg
KTH_B_qtd[5]=xn_ter
KTH_B_qtd[6]=et_n_prim
KTH_B_qtd[7]=et_n_seg
KTH_B_qtd[8]=et_n_ter
KTH_B_qtd[9]=es_n_prim
KTH_B_qtd[10]=es_n_seg
KTH_B_qtd[11]=es_n_ter
KTH_B_qtd[12]=mt_n_prim
KTH_B_qtd[13]=mt_n_seg
KTH_B_qtd[14]=mt_n_ter
KTH_B_qtd[15]=ms_n_prim
KTH_B_qtd[16]=ms_n_seg
KTH_B_qtd[17]=ms_n_ter
pprint(KTH_B_num)
pprint(KTH_B_class)
pprint(KTH_B_qtd)

arq_est_num = "/content/gdrive/My Drive/BancoDados/KTH_B_num.dat"
arq_est_class = "/content/gdrive/My Drive/BancoDados/KTH_B_class.dat"
arq_est_qtd = "/content/gdrive/My Drive/BancoDados/KTH_B_qtd.dat"
KTH_B_num.tofile(arq_est_num)
KTH_B_class.tofile(arq_est_class)
KTH_B_qtd.tofile(arq_est_qtd)

a_num = np.fromfile(arq_est_num)
b_class = np.fromfile(arq_est_class)
c_qtd = np.fromfile(arq_est_qtd)
pprint(a_num == KTH_B_num)
pprint(b_class == KTH_B_class)
pprint(c_qtd == KTH_B_qtd)
